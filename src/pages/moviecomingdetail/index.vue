  ...
  <section class="index_section">
    ...
    <div class="moviedetail_container" >
      <div class="moviedetail_content">
        <div class="movie_pic">
          <img
            :src="moviecomings.image"
            alt
          />
        </div>
        <div class="movie_con">
          <h2>{{moviecomings.titleCn}}</h2>
          <p class="movie_en">{{moviecomings.titleEn}}</p>
          <p class="movie_type"><b v-for="(item,index) in moviecomings.type" :key="index">/{{item}}</b></p>
      ...
      <div class="moviedetail_two">
        <p class="moviedetail_jieshao">{{moviecomings.commonSpecial}}</p>
        <p class="moviedetail_piao">查影讯/购票</p>
      </div>
      <div class="wenzi">
        <p>{{moviecomings.content}}</p>
      </div>
    </div>
  </section>
</template>
...
<script>
import { movieComingDetailApi } from "@api/movie";
export default {
  ...
  methods: {
    handleback() {
      this.$router.push("/movie/coming");
    }
  },
  async created() {
    let data = await movieComingDetailApi(this.id);
    this.moviecomings = data;
